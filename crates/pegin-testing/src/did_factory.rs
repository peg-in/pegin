...
///
/// A fresh coin is inserted with the wallet's puzzle hash; its `coin_id` becomes
/// the launcher id (`did:chia:<64-hex-char>`).  No real singleton CLVM runs —
/// that lives in feat-8.  This is enough for test scenarios that only need a
/// valid DID value in scope.
